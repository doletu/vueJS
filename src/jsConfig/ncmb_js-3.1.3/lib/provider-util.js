  ...
  Facebook.prototype.validate = function (authData) {
    if (!authData.id || !authData.access_token || !authData.expiration_date) {
      throw new Errors.InvalidOAuthDataError(
        "AuthData doesn't have any necessary parameter."
      );
    }
  };
  return Facebook;
...
var Apple = (function (superClass) {
  extend(Apple, superClass);
  function Apple() {
    this.providerName = "apple";
    return Apple.__super__.constructor.apply(this, arguments);
  }
  Apple.prototype.validate = function (authData) {
    if (!authData.id || !authData.access_token || !authData.client_id) {
      throw new Errors.InvalidOAuthDataError(
        "AuthData doesn't have any necessary parameter."
      );
    }
...
var ProviderUtil = (module.exports = {
  ...
  getProvider: function (authData, providerName) {
    if (Object.keys(authData).length === 0)
      throw new Errors.NoOAuthDataError("AuthData is required.");
    if (Object.keys(authData).length > 1 && !providerName)
      throw new Errors.NoProviderInfoError(
    ...
    if (!provider) {
      throw new Errors.UnknownAuthProviderError(
        "This ProviderName cannnot be used."
      );
    }
    return provider;
  },
