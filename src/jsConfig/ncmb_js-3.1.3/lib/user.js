...
var User = (module.exports = function (ncmb) {
  ...
  User.prototype.set = function (key, value) {
    ...
    if (isReserved(key))
      throw new Errors.UnReplaceableKeyError(
        key + " cannot be set, it is reserved."
      );
  ...
  User.prototype.signUpByAccount = User.prototype.save = function (callback) {
    if (!this.userName) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.NoUserNameError(
          "To login By Account, userName is necessary."
        )
      );
    }
    if (!this.password) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.NoPasswordError(
          "To login By Account, password is necessary."
        )
      );
    }
  ...
  User.prototype.signUpWith = function (provider, data, callback) {
    if (this.userName) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.ContentsConflictError(
          "To sign up with SNS account, user cannot have arbitrary userName."
        )
      );
    }
    ...
    if (!data && (!this.authData || Object.keys(this.authData).length !== 1)) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.InvalidAuthInfoError(
          "If provider and its data are not set, user must have an authData."
        )
      );
    }
    try {
    ...
    this.authData[provider.getName()] =
      this.authData[provider.getName()] || provider.getAuthData();
    return ncmb
      .request({
        path: "/" + ncmb.version + this.className,
        method: "POST",
  ...
  User.prototype.requestPasswordReset = function (callback) {
    if (!this.mailAddress) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.NoMailAddressError("MailAddress must be set.")
      );
    }
    return ncmb
      .request({
  ...
  User.prototype.update = function (callback) {
    if (!this.objectId) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.NoObjectIdError("Updated object must be saved before.")
      );
    }
  ...
  User.prototype.delete = function (callback) {
    if (!this.objectId) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.NoObjectIdError("Deleted object must be saved before.")
      );
    }
  ...
  User.login = function (userName, password, callback) {
    ...
    if (!userName || !password) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.NoAuthInfoError(
          "To login by account, userName and password are necessary."
        )
      );
    }
    if (!user) {
  ...
  User.loginWithMailAddress = function (mailAddress, password, callback) {
    ...
    if (!mailAddress || !password) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.NoAuthInfoError(
          "To login by account with mail address, mailAddress and password are necessary."
        )
      );
    }
    if (!user) {
  ...
  User.prototype.loginAsAnonymous = function (uuid, callback) {
    ...
    if (this.userName) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.ContentsConflictError(
          "To login as anonymous, user cannot have arbitrary userName."
        )
      );
    }
    if (this.authData && !this.authData.anonymous) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.InvalidAuthInfoError(
          "To login as anonymous, user cannot have other SNS authData."
        )
      );
    }
    if (!uuid) {
    ...
    if (!regexp.test(uuid)) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.InvalidFormatError("Uuid format is invalid.")
      );
    }
  ...
  User.loginWith = function (provider, data, callback) {
    ...
    if (provider instanceof ncmb.User) {
      ...
      if (!user.authData) {
        return (callback || Promise.reject.bind(Promise))(
          new Errors.NoOAuthDataError(
            "If user instance is set as first argument, it must have an authData."
          )
        );
      }
      if (Object.keys(user.authData).length === 1) {
        ...
        if (typeof data === "function") {
          callback = data;
          return (callback || Promise.reject.bind(Promise))(
            new Errors.NoProviderInfoError(
              "If User that have multiple SNS authData is set as first argument, provider must be specified as second argument."
            )
          );
        } else {
          provider = data;
  ...
  User.prototype.loginWith = function (provider, data, callback) {
    ...
    try {
      ...
      if (
        Object.keys(this.authData || data).indexOf(provider.getName()) === -1
      ) {
        throw new Errors.InvalidProviderError(
          "This provider cannot be used to login."
        );
      }
      if (data && data[provider.getName()]) {
        _data = this.authData || data;
        Object.keys(_data[provider.getName()]).forEach(
          function (key) {
            if (
              _data[provider.getName()][key] !== data[provider.getName()][key]
            ) {
              throw new Errors.InvalidOAuthDataError(
                "This OAuth data is invalid."
              );
            }
          }.bind(this)
        );
  ...
  User.prototype.logout = function (callback) {
    if (!this.sessionToken) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.NoSessionTokenError("This user doesn't login.")
      );
    }
    var currentSessionToken = ncmb.sessionToken;
    if (currentSessionToken !== this.sessionToken)
      ncmb.sessionToken = this.sessionToken;
  ...
  User.prototype.linkWith = function (provider, data, callback) {
    if (!provider) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.InvalidAuthInfoError("Provider is missing.")
      );
    }
    if (!data) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.InvalidAuthInfoError(
          "If provider and its data are not set, user can not link."
        )
      );
    }
    try {
  ...
  User.prototype.unLinkWith = function (provider, callback) {
    ...
    if (!this.authData) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.InvalidAuthInfoError(
          "User doesn't have authData, so can not unlink."
        )
      );
    }
    if (!this.authData[provider]) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.InvalidAuthInfoError(
          "User doesn't exist " + provider + " to unlink."
        )
      );
    }
    try {
