...
var GeoPoint = (module.exports = function (ncmb) {
  function GeoPoint(lat, lng) {
    var pos = GeoPoint.validate(lat, lng) ||
      ...
      (typeof lat === "object" &&
        GeoPoint.validate(lat.latitude, lat.longitude)) || {
        latitude: 0,
        longitude: 0,
      };
  ...
  GeoPoint.validate = function (latitude, longitude) {
    ...
    if (isNaN(latitude) || isNaN(longitude)) {
      throw new errors.OutOfRangeInputError(
        "GeoPoint latitude and longitude should be number"
      );
    }
    if (latitude < -90.0) {
      throw new errors.OutOfRangeInputError(
        "GeoPoint should not take latitude (" + latitude + ") < -90.0."
      );
    }
    if (latitude > 90.0) {
      throw new errors.OutOfRangeInputError(
    ...
    if (longitude < -180.0) {
      throw new errors.OutOfRangeInputError(
        "GeoPoint should not take longitude (" + longitude + ") < -180.0."
      );
    }
    if (longitude > 180.0) {
      throw new errors.OutOfRangeInputError(
