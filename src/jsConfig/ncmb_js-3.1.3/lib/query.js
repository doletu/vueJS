...
var Query = (module.exports = (function () {
  ...
  Query.prototype.near = function (key, location) {
    if (!(location instanceof this.ncmb.GeoPoint)) {
      throw new this.ncmb.Errors.InvalidArgumentError(
        "Second argument must be instance of ncmb.GeoPoint."
      );
    }
  ...
  Query.prototype.withinKilometers = function (key, location, maxDistance) {
    if (!(location instanceof this.ncmb.GeoPoint)) {
      throw new Errors.InvalidArgumentError(
        "Location must be instance of ncmb.GeoPoint."
      );
    }
   ...
   * @return {this}
   */
  Query.prototype.withinSquare = function (
    key,
    southWestVertex,
    northEastVertex
  ) {
    if (
      !(southWestVertex instanceof this.ncmb.GeoPoint) ||
      !(northEastVertex instanceof this.ncmb.GeoPoint)
    ) {
      throw new Errors.InvalidArgumentError(
        "Location must be instance of ncmb.GeoPoint."
      );
  ...
  Query.prototype.or = function (subqueries) {
    ...
    for (var i = 0; i < subqueries.length; i++) {
      if (!subqueries[i]._where)
        throw new Errors.InvalidArgumentError(
          "Argument is invalid. Input query or array of query."
        );
  ...
  Query.prototype.select = function (key, subkey, subquery) {
    ...
    if (!subquery._className)
      throw new Errors.InvalidArgumentError(
        "Third argument is invalid. Input query."
      );
    var className = null;
    ...
    if (subquery._limit > 0 && subquery._skip > 0) {
      this._where[key]["$select"] = {
        query: {
          className: className,
          where: subquery._where,
          limit: subquery._limit,
          skip: subquery._skip,
        },
        key: subkey,
      };
      ...
      this._where[key]["$select"] = {
        query: { className: className, where: subquery._where },
        key: subkey,
      };
    }
    return this;
  ...
  Query.prototype.inQuery = function (key, subquery) {
    ...
    if (!subquery._className)
      throw new Errors.InvalidArgumentError(
        "Second argument is invalid. Input query."
      );
    var className = null;
    if (subquery._className === "/users") {
      className = "user";
  ...
  Query.prototype.skip = function (skip) {
    if (typeof skip !== "number")
      throw new Errors.InvalidskipError("Skip must be number.");
    if (skip < 0)
      throw new Errors.InvalidskipError("Skip must be greater than 0.");
    this._skip = skip;
    return this;
  };
  ...
  Query.prototype.fetchById = function (id, callback) {
    ...
    if (typeof Klass === "undefined") {
      return Promise.reject(
        new Error("no class definition `" + this._className + "`")
      );
    }
  ...
  Query.prototype.fetchAll = function (callback) {
    ...
    if (Object.keys(this._where).length !== 0)
      opts.push("where=" + JSON.stringify(this._where));
