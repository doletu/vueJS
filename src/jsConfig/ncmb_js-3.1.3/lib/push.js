...
var Push = (module.exports = function (ncmb) {
    ...
    for (var attr in attrs) {
      if (attrs.hasOwnProperty(attr)) {
        if (!isReserved(attr)) {
          ...
          throw new ncmb.Errors.UnmodifiableVariableError(
            attr + " cannot be set, it is reserved."
          );
        }
      }
    }
  ...
  Push.prototype.update = function (callback) {
    ...
    if (
      ...
      (this.contentAvailable || this.badgeIncrementFlag)
    ) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.InvalidArgumentError(
          "badgeSetting must be null when set with contentAvailable or badgeIncrementFlag."
        )
      );
    }
    if (this.contentAvailable && this.badgeIncrementFlag) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.InvalidArgumentError(
          "Set true either contentAvailable or badgeIncrementFlag only."
        )
      );
    }
    if (
      this.badgeIncrementFlag &&
      this.target &&
      Array.isArray(this.target) &&
      this.target.indexOf("ios") == -1
    ) {
      return (callback || Promise.reject.bind(Promise))(
        new Errors.InvalidArgumentError(
          "badgeIncrementFlag cannot set without target 'ios'."
        )
      );
    }
