...
module.exports = function (opts, callback) {
  ...
  var headers = {
    ...
    "X-NCMB-SDK-Version": "javascript-3.1.3",
  };

  if (!file) headers["Content-Type"] = "application/json";
  if (this.sessionToken)
    headers["X-NCMB-Apps-Session-Token"] = this.sessionToken;
    ...
    function (resolve, reject) {
      var _callback = function (err, res) {
        ...
        if (res.statusCode >= 400 || res.status >= 400)
          return reject(res.error || new Error(res.text));
      ...
      var checkResponseValidate = function (err, res) {
        ...
        if (
          ...
          if (newSignature !== responseSignature) {
            var err = {
              code: "E100001",
              Error: "Authentication error by response signature incorrect.",
            };
        ...
        if (typeof data == "object") {
          ...
          if (typeof query == "object") {
            Object.keys(query).forEach(function (key) {
              //r.query(key + '=' + encodeURIComponent(query[key]).replace(/'/g, '%27'));
              r.query(key + "=" + fixedEncodeURIComponent(query[key]));
            });
          }
          if (responseType && r.responseType) r.responseType(responseType);
          var isNodeEv = new Function(
            "try { return this === global; } catch(e) { return false; }"
          );
          if (
            isNodeEv() &&
            method === "GET" &&
            path.indexOf("/2013-09-01/files/") != -1
          ) {
            if (r.buffer) {
