  ...
  function () {
    ...
    describe("プッシュ通知更新", function () {
      describe("update", function () {
        context(
          "存在するプッシュ通知のIDを指定し、更新に成功",
          function (done) {
            var updatePush = null;
            beforeEach(function () {
              updatePush = new ncmb.Push({
                objectId: "update_push_id",
                message: "updated",
              });
            });
            it("callback で取得できる", function (done) {
        ...
        it("badgeIncrementFlagがtrueのときに、targetをiosなしで更新しようとした場合、更新に失敗してエラーが返る", function (done) {
          ...
          updatePush.set("target", ["android"]).set("badgeIncrementFlag", true);
          updatePush
            .update()
            .then(function (updateRole) {
              done(new Error("error が返されなければならない"));
