...
describe("NCMB Relation", function () {
  ...
  describe("リレーションの追加", function () {
    ...
    describe("add", function () {
      context(
        "Data型のオブジェクトをリレーションに追加して保存し、レスポンスを",
        function () {
          var Food = null;
          var food = null;
          beforeEach(function () {
      ...
      context(
        "User型のオブジェクトをリレーションに追加して保存し、レスポンスを",
        function () {
          var subuser = null;
          beforeEach(function () {
            relation = new ncmb.Relation();
      ...
      context(
        "Role型のオブジェクトをリレーションに追加して保存し、レスポンスを",
        function () {
          var subrole = null;
          beforeEach(function () {
            relation = new ncmb.Relation();
      ...
      context(
        ...
        function () {
          ...
          beforeEach(function () {
            Food = ncmb.DataStore("food");
            food = new Food([
              { name: "orange", type: "fruit", status: "success" },
            ]);
          });
  ...
  describe("リレーションの削除", function () {
    ...
    describe("remove", function () {
      context(
        "Data型のオブジェクトをリレーションから削除して保存し、レスポンスを",
        function () {
          var Food = null;
          var food = null;
          beforeEach(function () {
      ...
      context(
        "User型のオブジェクトをリレーションから削除して保存し、レスポンスを",
        function () {
          var subuser = null;
          beforeEach(function () {
            relation = new ncmb.Relation();
            ...
            mainobj = new MainObj();
            if (ncmb.stub)
              user_callback_id = user_promise_id = "remove_user_id";
            subuser = new ncmb.User({
              userName: "Yamada Tarou",
              password: "password",
            });
      ...
      context(
        "Role型のオブジェクトをリレーションから削除して保存し、レスポンスを",
        function () {
          var subrole = null;
          beforeEach(function () {
            relation = new ncmb.Relation();
            ...
            mainobj = new MainObj();
            if (ncmb.stub)
              role_callback_id = role_promise_id = "remove_role_id";
            subrole = new ncmb.Role("role_name");
            subrole.objectId = role_related_id;
          });
          it("callback で取得できる", function (done) {
      ...
      context(
        "リレーションから削除する複数のオブジェクトを配列で指定して保存し、レスポンスを",
        function () {
          var Food = null;
          var food1 = null;
          var food2 = null;
          ...
          beforeEach(function () {
            ...
            mainobj = new MainObj();
            if (ncmb.stub)
              array_callback_id = array_promise_id = "remove_multi_id";
            Food = ncmb.DataStore("food");
            food1 = new Food({
              name: "orange",
              type: "fruit",
      ...
      it("複数回実行することで、削除するオブジェクトを追加できる", function (done) {
        ...
        var relation = new ncmb.Relation();
        relation.remove(food1).remove(food2);
        try {
          expect(relation.objects.length).to.be.eql(2);
          done();
        } catch (err) {
      ...
      context(
        "インスタンス生成時にリレーション要素を削除するクラス名を指定し、",
        function () {
          var Food = null;
          var food = null;
          var relation = null;
