  ...
  describe("更新オペレーション設定", function () {
      ...
      it("keyとamountを指定してsetIncrementを利用し、オブジェクトを新規保存すると、keyのプロパティにIncrementした結果の数が設定される", function (done) {
        ...
        objForIncrementTest.save(function (err, obj) {
          if (err) {
            ...
            classForIncrementTest.fetchById(
              validObjIdThatIsSetIncrementWithAmount,
              function (err, obj) {
                if (err) {
                  done(err);
                } else {
      ...
      it("keyのみを指定した場合、amountが1になる", function (done) {
        ...
        objForIncrementTest.save(function (err, obj) {
          if (err) {
            ...
            classForIncrementTest.fetchById(
              validObjIdThatIsSetIncrementWithoutAmount,
              function (err, obj) {
                if (err) {
                  done(err);
                } else {
      ...
      it("複数回setIncrementを実行した場合、amountの合計値が保存される", function (done) {
        ...
        objForIncrementTest.save(function (err, obj) {
          if (err) {
            ...
            classForIncrementTest.fetchById(
              validObjIdThatIsSetIncrementMultiple,
              function (err, obj) {
                if (err) {
                  done(err);
                } else {
      ...
      it("メソッドチェインで連続実行できる", function (done) {
        ...
        objForIncrementTest.save(function (err, obj) {
          if (err) {
            ...
            classForIncrementTest.fetchById(
              validObjIdThatIsSetIncrementWithMethodChain,
              function (err, obj) {
                if (err) {
                  done(err);
                } else {
      ...
      it("同じキーに複数のオペレーションが設定された場合は上書きする", function (done) {
        ...
        objForIncrementTest.save(function (err, obj) {
          if (err) {
            ...
            classForIncrementTest.fetchById(
              validObjIdThatIsOverrideOperation,
              function (err, obj) {
                if (err) {
                  done(err);
                } else {
    ...
    context("addUnique", function () {
      ...
      it("重複する値が入力されたとき、エラーが返る", function (done) {
        expect(function () {
          user.addUnique("key", [1, 2, 3]).addUnique("key", 1);
        }).to.throw(Error);
        done();
      });
    });
